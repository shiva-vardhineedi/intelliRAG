...
# Configure Redis client and RQ queue
redis_client = redis.Redis(host='localhost', port=6379, db=0)
queue = Queue('query_queue', connection=redis_client)
redis_client = redis.Redis(host='localhost', port=6379, db=0)
...
def create_index(directory, pdf_files):
    ...
    # If only one file is selected
        ...
        progress_bar.progress((pdf_files.index(pdf_file) + 1) / len(pdf_files))
        ...
        if selected_index_file:
            ...
            if st.button("Search Answer") and user_question:
                if os.path.exists(index_path) and os.path.exists(chunks_path):
                    ...
                    job = queue.enqueue(query_system, user_question, index_path, chunks_path)
                    st.write(f"Your query has been added to the queue. Job ID: {job.id}")
                else:
                ...
                if index is not None and text_chunks is not None:
                    with st.spinner("Searching for the best answers..."):
                        try:
                            answer = query_system(user_question, index, text_chunks)
                            st.write(f"Answer:\n\n{answer}")
                        except Exception as e:
